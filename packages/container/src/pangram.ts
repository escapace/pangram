...
import { reduceGraph, schemaFontPropertiesKeys } from './state/flatten-configuration'
import type { CSSProperties, WebFont, WebFontLocale, WebFontsJson } from './state/user-schema'
import type { FontProperties, FontStateWritten, Options, State, Style } from './types'
...
export const pangram = async (options: Options = {}) => {
  ...
  for (const style of state.configuration.styles) {
    ...
    const primaryFontMetrics =
      (primaryFont === undefined
        ? undefined
        : fontMetrics((await fontOpen(primaryFont, fontProperties, state)).font)) ??
        ...
        await fontFace(
          {
            adjustments: fontAdjust(
              primaryFontMetrics,
              fontMetrics((await fontOpen(secondaryFont, fontProperties, state)).font),
            ),
            font: secondaryFont,
            fontProperties,
            publicPath: state.publicPath,
